...
for i=1:length(T)
    ...
    B2X=[so(1,2), st(1,2), so(1,3), st(1,1), st(1,5)];
    B2Y=[so(2,2), st(2,2), so(2,3), st(2,1), st(2,5)];
    B2Z=[so(2,2), st(2,2), so(3,3), st(3,1), st(3,5)];   

    B3X=[so(1,4), so(1,6),st(1,6), so(1,7),st(1,5), so(1,5),];
    B3Y=[so(2,4), so(2,6),st(2,6), so(2,7),st(2,5), so(2,5),];
    B3Z=[so(3,4), so(3,6),st(3,6), so(3,7),st(3,5), so(3,5),];
    
    t=T(i);
    t=num2str(t);
    axes(handles.axes1)
    plot(B1X,B1Y,'.-',B2X,B2Y,'.-',B3X,B3Y,'.-','Markersize',23,'linewidth',4);
