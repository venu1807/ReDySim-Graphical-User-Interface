...
for i=1:length(T)
    ...
    B1X=[so(1,1), st(1,1), so(1,2), st(1,2)];
    B1Y=[so(2,1), st(2,1), so(2,2), st(2,2)];
    B1Z=[so(3,1), st(3,1), so(3,2), st(3,2)];
    B2X=[ so(1,3), st(1,3)];
    B2Y=[ so(2,3), st(2,3)];
    B2Z=[ so(3,3), st(3,3)];   
    ...
    t=num2str(t);
    axes(handles.axes1)
    plot3(B1X,B1Y,B1Z,'.-',B2X,B2Y, B2Z,'rs','linewidth',4,'MarkerSize',24);
