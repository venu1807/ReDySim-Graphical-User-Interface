...
for i=1:length(T)
    ...
    plot3(BZ,BY,BX,'.-',L1Z,L1Y,L1X,'.-',L2Z,L2Y,L2X,'.-','Markersize',25,'linewidth',4);
    axis([ xmin xmax  ymin ymax zmin zmax]);
    set (gca,'fontsize',10,'fontweight','normal','fontname','times new romans','linewidth',0.5,'Box', 'off','TickDir','out' );
    xlabel('Z (m)','fontweight','normal','fontsize',10);
    ylabel('Y (m)','fontweight','normal','fontsize',10);
    zlabel('X (m)','fontweight','normal','fontsize',10);
    title(['Current time t=',t],'fontweight','normal','fontsize',12);
%     title(['Current time t=',t],'fontweight','normal','fontsize',12)
    %view(0,0);
    grid on; 
    drawnow;
